...
void read_cifar10_dataset(const std::string &data_dir_path)
{
	for (int i = 1; i <= 5; i++) {
		tiny_dnn::parse_cifar10(data_dir_path + "/data_batch_" + std::to_string(i) + ".bin",
			&train_images, &train_labels, 0.0, 1.0, 0, 0, 0.5, 0.2);
	}
...
void learning_and_test_cifar10_dataset(torch::Device device)
{
	...
	model.get()->add_fc(10);
#else
	model.get()->add_conv2d(3, 64, 1, 1, 1);
	model.get()->add_bn();
	model.get()->add_ReLU();
	...
	model.get()->add_conv2d(64, 64, 5, 5, 5);
	model.get()->add_bn();
	model.get()->add_ReLU();

	model.get()->add_dropout(0.25);
	model.get()->add_maxpool2d(2);

	//
	model.get()->add_conv2d(64, 128, 1, 1, 1);
	model.get()->add_bn();
	model.get()->add_ReLU();
	...
	model.get()->add_conv2d(128, 128, 5, 5, 5);
	model.get()->add_bn();
	model.get()->add_ReLU();
	...
	//
	model.get()->add_conv2d(128, 256, 1, 1, 1);
	model.get()->add_bn();
	model.get()->add_ReLU();

	model.get()->add_conv2d(256, 256, 3, 3, 3);
	model.get()->add_bn();
	model.get()->add_ReLU();

	model.get()->add_conv2d(256, 256, 5, 5, 5);
	model.get()->add_bn();
	model.get()->add_ReLU();
